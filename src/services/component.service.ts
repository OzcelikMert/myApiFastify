...
const get = async (params: IComponentGetParamService) => {
  ...
  const doc = await query.lean<IComponentModel>().exec();

  if (doc) {
    for (const docElement of doc.elements) {
      docElement.contents = [];
    }
  }
...
const getMany = async (params: IComponentGetManyParamService) => {
  ...
  const docs = await query.lean<IComponentModel[]>().exec();
...
const getDetailed = async (params: IComponentGetDetailedParamService) => {
  ...
  const doc = await query.lean<IComponentGetDetailedResultService>().exec();
  ...
  if (doc) {
    for (const docElement of doc.elements) {
      if (Array.isArray(docElement.contents)) {
        ...
        docElement.contents =
          docElement.contents.findSingle('langId', params.langId) ??
          docElement.contents.findSingle('langId', defaultLangId);
      }
    }
  }
...
const getManyDetailed = async (
  params: IComponentGetManyDetailedParamService
) => {
  let filters: mongoose.FilterQuery<IComponentModel> = {};
  params = MongoDBHelpers.convertToObjectIdData(params, componentObjectIdKeys);
  const defaultLangId = MongoDBHelpers.convertToObjectId(Config.defaultLangId);
  ...
  const docs = await query.lean<IComponentGetDetailedResultService[]>().exec();
  ...
  if (doc) {
    if (params.elements) {
      ...
      doc.elements = doc.elements.filter(
        (docElement) =>
          params.elements &&
          params.elements.indexOfKey('_id', docElement._id) > -1
      );
      // Check Update
      for (const paramElement of params.elements) {
        let docElement = doc.elements.findSingle('_id', paramElement._id);
        if (docElement) {
          let docElementContent = docElement.contents.findSingle(
            'langId',
            paramElement.contents.langId
          );
          if (docElementContent) {
            docElementContent = Object.assign(
              docElementContent,
              paramElement.contents
            );
          } else {
