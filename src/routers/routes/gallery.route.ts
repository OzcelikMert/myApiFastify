...
export const galleryRoute = function (fastify: FastifyInstance, opts: any, done: () => void) {
    ...
    fastify.delete(galleryEndPoint.DELETE_IMAGE, { preHandler: [RequestMiddleware.check(GallerySchema.deleteMany), SessionAuthMiddleware.check, GalleryMiddleware.checkMany, GalleryMiddleware.checkIsAuthorMany] }, GalleryController.deleteManyImage);
    done();
}