...
export const postRoute = function (fastify: FastifyInstance, opts: any, done: () => void) {
    ...
    fastify.post(postEndPoint.ADD, { preHandler: [RequestMiddleware.check(PostSchema.post), SessionAuthMiddleware.check, PermissionMiddleware.check(PermissionUtil.getPostPermission), PostMiddleware.checkPageTypeIdWithId] }, PostController.add);
