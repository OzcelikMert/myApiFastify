...
export const postRoute = function (fastify: FastifyInstance, opts: any, done: () => void) {
    ...
    fastify.put(postEndPoint.UPDATE_RANK_WITH_ID, { preHandler: [RequestMiddleware.check(PostSchema.putRankWithId), SessionAuthMiddleware.check, PermissionMiddleware.check(PermissionUtil.getPostPermission), PostMiddleware.checkWithId, PostMiddleware.checkWithIdIsAuthor] }, PostController.updateRankWithId);
    fastify.put(postEndPoint.UPDATE_STATUS, { preHandler: [RequestMiddleware.check(PostSchema.putStatusMany), SessionAuthMiddleware.check, PermissionMiddleware.check(PermissionUtil.getPostPermission), PostMiddleware.checkMany, PostMiddleware.checkManyIsAuthor] }, PostController.updateStatusMany);
    fastify.put(postEndPoint.UPDATE_WITH_ID, { preHandler: [RequestMiddleware.check(PostSchema.putWithId), SessionAuthMiddleware.check, PermissionMiddleware.check(PermissionUtil.getPostPermission), PostMiddleware.checkWithId, PostMiddleware.checkWithIdIsAuthor] }, PostController.updateWithId);
