...
export const postRoute = function (fastify: FastifyInstance, opts: any, done: () => void) {
    ...
    fastify.put(postEndPoint.UPDATE_VIEW_WITH_ID, { preHandler: [RequestMiddleware.check(PostSchema.putWithIdView), ViewMiddleware.check, PostMiddleware.checkWithId] }, PostController.updateWithIdView);
    fastify.put(postEndPoint.UPDATE_RANK_WITH_ID, { preHandler: [RequestMiddleware.check(PostSchema.putWithIdRank), SessionAuthMiddleware.check, PermissionMiddleware.check(PermissionUtil.getPostPermission), PostMiddleware.checkWithId, PostMiddleware.checkWithIdIsAuthor] }, PostController.updateWithIdRank);
