...
export const sitemapRoute = function (fastify: FastifyInstance, opts: any, done: () => void) {
    ...
    fastify.get(endPoint.GET_POST, { preHandler: [RequestMiddleware.check(SitemapSchema.getPost)] }, SitemapController.getPost);
    fastify.get(endPoint.GET_POST_TERM, { preHandler: [RequestMiddleware.check(SitemapSchema.getPostTerm)] }, SitemapController.getPostTerm);
    done();
}