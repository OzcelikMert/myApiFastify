  ...
  await LogMiddleware.error(req, reply, async () => {
    ...
    if (
      ...
      if (
        ...
        !(serviceResult.authors as ObjectId[])?.some(
          (author) =>
            author.toString() == req.sessionAuth!.user?.userId.toString()
        )
      ) {
        apiResult.status = false;
  ...
  await LogMiddleware.error(req, reply, async () => {
    ...
    if (
      ...
      for (const post of serviceResult) {
        if (
          post.authorId.toString() != req.sessionAuth!.user?.userId.toString()
        ) {
          apiResult.status = false;
          apiResult.setErrorCode = ApiErrorCodes.noPerm;
          apiResult.setStatusCode = ApiStatusCodes.forbidden;
...
const checkPermissionForPageWithId = async (
  req: FastifyRequest,
  reply: FastifyReply
) => {
  await LogMiddleware.error(req, reply, async () => {
    ...
    if (
      reqData.body.typeId == PostTypeId.Page &&
      !PermissionUtil.checkPermissionRoleRank(
        req.sessionAuth!.user!.roleId,
...
const checkUserRoleForPage = async (
  req: FastifyRequest,
  reply: FastifyReply
) => {
  await LogMiddleware.error(req, reply, async () => {
