...
const getManyImage = async (req: FastifyRequest, reply: FastifyReply) => {
  await LogMiddleware.error(req, reply, async () => {
    const apiResult = new ApiResult<IGalleryGetDetailedResultService[]>();
    apiResult.data = [];

    const reqData = req as IGalleryGetManySchema;

    const gallery = await GalleryService.getManyDetailed({
      ...reqData.query,
...
const addImage = async (req: FastifyRequest, reply: FastifyReply) => {
  await LogMiddleware.error(req, reply, async () => {
    const apiResult = new ApiResult<IGalleryModel[]>();
    apiResult.data = [];
      ...
      upload(req, reply, async function (err: any) {
        ...
        try {
          ...
          if (file) {
            ...
            const imageProperties = await getImageProperties(name);

            const insertedData = await GalleryService.add({
...
const deleteManyImage = async (req: FastifyRequest, reply: FastifyReply) => {
  await LogMiddleware.error(req, reply, async () => {
    ...
    const galleryItems = await GalleryService.getMany({
