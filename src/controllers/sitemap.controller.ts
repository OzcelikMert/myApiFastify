...
import {
  ISitemapGetPostSchema,
  ISitemapGetPostTermSchema,
} from '@schemas/sitemap.schema';
import {
...
const getMaps = async (req: FastifyRequest, reply: FastifyReply) => {
  await LogMiddleware.error(req, reply, async () => {
    const apiResult = new ApiResult<ISitemapGetMapsResultService>();

    apiResult.data = {
      post: await SitemapService.getPostCount({
        typeId: [PostTypeId.Page, PostTypeId.Portfolio, PostTypeId.Blog],
      }),
...
const getPost = async (req: FastifyRequest, reply: FastifyReply) => {
  await LogMiddleware.error(req, reply, async () => {
    const apiResult = new ApiResult<ISitemapGetPostResultService[]>();
...
const getPostTerm = async (req: FastifyRequest, reply: FastifyReply) => {
  await LogMiddleware.error(req, reply, async () => {
    const apiResult = new ApiResult<ISitemapGetPostTermResultService[]>();
